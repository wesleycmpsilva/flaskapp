...
def get_auth_headers2(url_list, system_name):
    ...
    for item in url_list:
        ...
        try:
            response = requests.get(service_url, headers=headers, auth=auth, verify=True)
            output += f"{response.status_code} for {url}\n"
            output += response.text[:150] + "\n"
            response = requests.get(service_url, headers=headers, auth=auth, verify=False)
            output += f"{response.status_code} for {url}\n"
            output += response.text[:150] + "\n"
        ...
        try:
            response = requests.get(service_alias, headers=headers, auth=auth, verify=True)
            output += f"{response.status_code} for {alias}\n"
            output += response.text[:150] + "\n"
            response = requests.get(service_alias, headers=headers, auth=auth, verify=False)
            output += f"{response.status_code} for {alias}\n"
            output += response.text[:150] + "\n"
